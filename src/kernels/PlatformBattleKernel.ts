...
// Aethelgard Platform Battle Kernel v2.0 (PURE AJS)
...
function on_combat_event(op, sender, p1, p2, p3) {
    if (op == EVT_DAMAGE) {
        // Platform uses EVT_DAMAGE via combat_events: p1 = Src, p2 = Target, p3 = Dmg
        let srcId = p1;
        let tgtId = p2;
        let dmg = p3;

        if (rpg_table[tgtId].state == 0) { // Alive
            rpg_table[tgtId].hp -= dmg;

            if (rpg_table[tgtId].hp <= 0) {
                rpg_table[tgtId].hp = 0;
                rpg_table[tgtId].state = 1; // Dead
                Chan("BUS") <- [EVT_DEATH, tgtId, rpg_table[tgtId].invItem, 0];
                Log("Entity Died:");
                Log(tgtId);
                if (tgtId == 0) {
                    Log("GAME OVER");
                    // Ensure player kernel is also synced
                    // EVT_DEATH to BUS is caught by PlatformKernel to disable movement
                }
            }
        }
    }
...
export const PLATFORM_BATTLE_KERNEL_BLOCKS = [
  ...STANDARD_KERNEL_FIRMWARE,
  BLOCK_STANDARD_INBOX,
  AetherTranspiler.transpile(AJS_LOGIC, KernelID.PLATFORM_BATTLE),
  ": INIT_BATTLE INIT_BATTLE_LOGIC AJS_INIT_CHANNELS ' HANDLE_EVENTS HANDLE_EVENTS_XT ! ;",
  ": RUN_BATTLE_CYCLE RUN_BATTLE_STEP ;"
];

export const PLATFORM_BATTLE_AJS_SOURCE = AJS_LOGIC;
export const PLATFORM_BATTLE_FORTH_SOURCE = PLATFORM_BATTLE_KERNEL_BLOCKS.join("\n");
