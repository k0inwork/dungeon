...
// Aethelgard Platform Hive AI Kernel v3.0 (PURE AJS)
...
function decide_action(id) {
  // In Platformer mode, AI movement and logic is handled continuously
  // by PlatformKernel.ts (e.g. frog_ai).
  // We leave this shell for standard Hive entity tracking but do not issue
  // discrete GRID requests.
}

function on_bus_event(op, sender, p1, p2, p3) {
  // EVT_COLLIDE is not used in Platform mode.
}
...
export const PLATFORM_HIVE_KERNEL_BLOCKS = [
  ...STANDARD_KERNEL_FIRMWARE,
  BLOCK_STANDARD_INBOX,
  AetherTranspiler.transpile(AJS_LOGIC, KernelID.PLATFORM_HIVE),
  ": INIT_HIVE INIT_HIVE_LOGIC AJS_INIT_CHANNELS ' HANDLE_EVENTS HANDLE_EVENTS_XT ! ;",
  ": RUN_HIVE_CYCLE RUN_HIVE_STEP ;"
];

export const PLATFORM_HIVE_AJS_SOURCE = AJS_LOGIC;
export const PLATFORM_HIVE_FORTH_SOURCE = PLATFORM_HIVE_KERNEL_BLOCKS.join("\n");
