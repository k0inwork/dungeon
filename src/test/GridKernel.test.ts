...
describe('GridKernel Logic Tests', () => {
  ...
  test('Map Initialization', () => {
    runner.run('INIT_MAP');
    runner.run('ENTITY_COUNT @ 0 JS_ASSERT');
  });

  test('Spawn and Find Entity', () => {
    runner.run('5 5 16711680 64 1 SPAWN_ENTITY');
    runner.run('5 5 FIND_ENTITY_AT 0 JS_ASSERT');
  });

  test('O(1) Lookup Verification', () => {
      runner.run('10 10 255 66 1 SPAWN_ENTITY');
      runner.run('10 10 FIND_ENTITY_AT 1 JS_ASSERT');
  });

  test('Move Entity and Update Map', () => {
      // Move entity 0 by dx=1, dy=0 (from 5,5 to 6,5)
      runner.run('0 1 0 MOVE_ENTITY');
      runner.run('6 5 FIND_ENTITY_AT 0 JS_ASSERT');
      runner.run('5 5 FIND_ENTITY_AT -1 JS_ASSERT');
  });
});
