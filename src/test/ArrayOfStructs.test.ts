...
test('transpiles array of structs', () => {
  ...
  expect(forth).toMatch(/5\s+NPCS\s+1\s+SIZEOF_NPC\s+\*\s+\+\s+OFF_A\s+\+\s+!/);

  // Check for let k = npcs[2]
  expect(forth).toMatch(/NPCS\s+2\s+SIZEOF_NPC\s+\*\s+\+\s+LV_TEST_K\s+!/);

  // Check for k.b = 10
  expect(forth).toMatch(/10\s+LV_TEST_K\s+@\s+OFF_B\s+\+\s+!/);
});
...
test('transpiles local array of structs', () => {
  ...
  // Check for access
  expect(forth).toMatch(/42\s+LV_TEST_LOCAL_NPCS\s+0\s+SIZEOF_NPC\s+\*\s+\+\s+OFF_A\s+\+\s+!/);
});
...
test('transpiles exported struct and struct-function syntax', () => {
  ...
  // NPC(1).a = 5 -> 5 1 NPCS1 SWAP SIZEOF_NPC * + OFF_A + !
  expect(forth).toMatch(/5\s+1\s+NPCS1\s+SWAP\s+SIZEOF_NPC\s+\*\s+\+\s+OFF_A\s+\+\s+!/);
